...
def read_access_log_window(
    ...
    with access_log.open("r", encoding="utf-8") as file:
        for line in file:
            ...
            if request_data:
                match = timestamp_pattern.search(request_data["request_dt"])

                if not match:
                    continue

                access_log_dt = datetime.strptime(
                    match.group(1), "%d/%b/%Y:%H:%M:%S %z"
                )

                if access_log_dt_read_start <= access_log_dt <= access_log_dt_read_end:
